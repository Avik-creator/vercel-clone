...
pub async fn refresh(State(_state): State<AppState>) -> AppResult<Json<serde_json::Value>> {
    Err(AppError::BadRequest("not yet implemented".into()))
}

pub async fn github_oauth_redirect(State(state): State<AppState>) -> Redirect {
...
pub async fn github_oauth_callback(
    ...
    let redirect_url = format!(
        "{}/auth/callback?token={}",
        state.config.frontend_url, resp.token
    );
    Ok(Redirect::to(&redirect_url))
}

pub async fn me(State(state): State<AppState>, AuthUser(user): AuthUser) -> AppResult<Json<User>> {
